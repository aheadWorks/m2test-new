        ...
        if len(composer["require"]) > 0:
            for ext in composer["require"]:
                if ext.find('aheadworks') != -1:
                    proc = subprocess.Popen(['php', '-d', 'memory_limit=4G', '/usr/local/bin/composer', 'require', str(ext + ':' + composer["require"][ext])])
                    proc.communicate()
                    if proc.returncode != 0:
                        module_name = ext.split('/')[1]
                        proc = subprocess.Popen(['php', '-d', 'memory_limit=4G', '/usr/local/bin/composer', 'config', 'repositories.aheadworks', 'vcs',
